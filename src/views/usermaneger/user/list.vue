<template>
  <div class="app-container">
    ...
    <el-table border stripe v-loading="listLoading" :data="users" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="uface" label="头像" width="101" align="center">
        <template slot-scope="{row}">


          <el-image :src="row.uface" style="width: 80px; height: 80px;"></el-image>


        </template>
      </el-table-column>
      <el-table-column prop="uname" label="用户姓名" align="center" />
      <el-table-column prop="unick" label="用户昵称" align="center" />
      <el-table-column prop="utel" label="电话号码" align="center" />

      <el-table-column prop="umail" label="用户邮箱" align="center" />
      <el-table-column prop="uscore" label="用户评分" align="center" />


      <el-table-column label="操作" width="230" align="center">
        <template slot-scope="{row}">
          <el-button type="primary" size="medium" icon="el-icon-edit"  @click="showUpdateUser(row)" >修改信息</el-button>
    ...
    <!-- 对话框的结构 -->
    <el-dialog title="添加用户" :visible.sync="dialogUserVisible" :before-close="cancel">
      <el-form ref="user" :model="user" label-width="120px">
        <el-form-item label="用户id" prop="uid">
          <el-input v-model="user.uid" />
        </el-form-item>
        <el-form-item label="用户密码" prop="upwd">
          <el-input v-model="user.upwd" />
        </el-form-item>
        <el-form-item label="用户名" prop="uname">
          <el-input v-model="user.uname" />
        </el-form-item>
        <el-form-item label="用户昵称">
          <el-input v-model="user.unick" />
        </el-form-item>
        <el-form-item label="电话号码">
          <el-input v-model="user.utel" />
        </el-form-item>
    ...
    <el-dialog title="修改信息" :visible.sync="updataVisible" :before-close="cancelupdata">
      <el-form ref="updataForm" :model="user" label-width="120px">
        ...
        <el-form-item label="用户评分">
          <el-input v-model="user.uscore" :disabled="true" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :loading="loading" v-if="user.uid" type="primary" @click="Update">确 定</el-button>
      </div>
    </el-dialog>
    ...
    methods: {
      ...
      */
      async getUsers(page = 1) {
        ...
        const result = await this.$API.user.getUserList(this.page, limit, searchObj.username)
        console.log(result);
      ...
      */
      add() {
        ...
        this.loading = true
        this.$API.user.add(this.$store.state.user.token, user).then((result) => {
          this.loading = false
