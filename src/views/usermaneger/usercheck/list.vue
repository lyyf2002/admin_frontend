<template>
  <div class="app-container">
    ...
    <el-table border stripe v-loading="listLoading" :data="users" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="umail" label="电子邮箱" align="center"/>

      <el-table-column label="操作" width="250" align="center">
        <template slot-scope="{row}">
          <el-button type="success" size="medium" icon="el-icon-check"  @click="accept(row)" >通过</el-button>
          <el-button type="danger" size="medium" icon="el-icon-close"  @click="refuse(row)" >拒绝</el-button>
    ...
    methods: {
      ...
      */
      ...
      async accept(u) {
        ...
        await this.$API.user.accept(this.$store.state.user.token,u.acid)
      ...
      async refuse(u) {
        console.log(u.acid)
        console.log(this.$store.state.user.token)
        await this.$API.user.refuse(this.$store.state.user.token,u.acid)
      ...
      async acceptBatch() {
        ...
        while(i>=0){
          ...
          await this.$API.user.accept(this.$store.state.user.token,this.selectedIds[i])
          i --
        }
        this.selectedIds = []
        this.$message.success('通过成功')
      ...
      async refuseBatch() {
        ...
        while(i>=0){
          console.log(this.selectedIds[i])
          await this.$API.user.refuse(this.$store.state.user.token,this.selectedIds[i])
          i --
        }
        this.selectedIds = []
        this.$message.success('拒绝成功')
      ...
      */
      handleSelectionChange(selection) {
        ...
        this.selectedIds = selection.map(item => item.acid)
        console.log(this.selectedIds)
      },
      ...
      */
      async getUsers(page = 1) {
        ...
        const result = await this.$API.user.getaccountCheckPageList(this.page, limit, searchObj.username)
        console.log(result);
