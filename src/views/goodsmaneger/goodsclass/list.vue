<template>
  <div class="app-container">
    <el-form inline>
      <!-- 表单元素 -->
      <el-form-item>
        <el-input v-model="tempSearchObj.username" placeholder="类别名" />
      </el-form-item>
      <!-- 查询与情况的按钮 -->
    ...
    <div style="margin-bottom: 20px">
      ...
      <el-button type="primary" @click="showadd" >
        添 加</el-button>
      <el-button type="danger" @click="refuseBatch" :disabled="selectedIds.length===0">批量删除</el-button>
    </div>
    ...
    <el-table border stripe v-loading="listLoading" :data="users" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="gclassname" label="类别" align="center" />


      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{row}">
          <el-button type="success" size="medium" icon="el-icon-setting"  @click="updata(row)" >修改</el-button>

          <el-popconfirm :title="`确定删除 ${row.gclassname} 吗?`" @onConfirm="refuse(row)">
            <el-button style="margin-left:10px" slot="reference" type="danger" size="medium" icon="el-icon-delete"
              >删除</el-button>
          </el-popconfirm>
        </template>
      </el-table-column>
...
<el-dialog title="添加类别" :visible.sync="dialogaddVisible" :before-close="cancel">
      <el-form ref="user" :model="user"  label-width="120px" >
        <el-form-item label="类别名" prop="uid">
          <el-input v-model="user.gclassname" />
        </el-form-item>
      </el-form>
...
<el-dialog title="修改类别" :visible.sync="dialogupdataVisible" :before-close="cancelupdata">
      <el-form ref="user" :model="user"  label-width="120px" >
        <el-form-item label="类别名" prop="uid">
          <el-input v-model="user.gclassname" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :loading="loading"  type="primary" @click="updataclass">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        total: 0, // 总数量
        user: {}, // 当前要操作的user
        dialogaddVisible: false, // 是否显示用户添加/修改的dialog
    ...
    methods: {
      ...
      */
      ...
      updataclass(){
        ...
        this.loading = true
        this.$API.goods.update(user).then((result) => {
          this.loading = false
      ...
      add(){
        ...
        this.loading = true
        this.$API.goods.add(user).then((result) => {
          this.loading = false
      ...
      async refuse(u) {
        ...
        await this.$API.goods.refuse(u.gclassid)
      ...
      async refuseBatch() {
        ...
        while(i>=0){
          ...
          await this.$API.goods.refuse(this.selectedIds[i])
          i --
        }
        this.selectedIds = []
        this.$message.success('删除成功')
      ...
      */
      handleSelectionChange(selection) {
        ...
        this.selectedIds = selection.map(item => item.gclassid)
        // console.log(this.selectedIds)
      },
      ...
      */
      async getUsers(page = 1) {
        ...
        const result = await this.$API.goods.getgoodsclassPageList(this.page, limit, searchObj.username)
        console.log(result);
