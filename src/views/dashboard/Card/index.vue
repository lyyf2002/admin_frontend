  ...
  <div >
    <el-row :gutter="10">
      <el-col :span="6">
        <el-card>
          <!-- 第一个card -->
          <Detail title="总销售额" :count="this.totalMoney">
      ...
      <el-col :span="6">
        <el-card>
          <!-- 第二个card -->
          <Detail title="访问量" :count="this.totalVisitNum">
             <template slot="charts">
                 <lineCharts></lineCharts>
             </template>
            <template slot="footer">
              <span>日访问量 {{this.curVisitNum}}</span>
            </template>
          </Detail>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          <!-- 第三个card -->
          <Detail title="交易笔数"
          :count="this.totalOrderNum">
            <template slot="charts">
               <barCharts/>
            </template>
            <template slot="footer">
              <span>今日交易数 {{this.curOrderNum}}</span>
            </template>
          </Detail>
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card>
          <!-- 第四个card -->
          <Detail title="活跃比" :count="this.act">
             <template slot="charts">
                <progressCharts :d="this.act"/>
             </template>
            <template slot="footer">
              <span
                >总人数&nbsp;&nbsp;{{this.allpeople}}
...
export default {
  ...
  created() {
    ...
    this.getVisitNum()
    this.getActData()
    // setInterval(this.getMoneys, 5000);
  },
  methods:{
    async getMoneys(){
      const result = await this.$API.dashboard.getMoneys()
      this.totalMoney = result.data.totalMoney
      this.weekrate = result.data.weekrate
      this.dayrate = result.data.dayrate
      this.daymoney = result.data.daymoney

    },

    async getOrderNum(){
      const result = await this.$API.dashboard.getOrderNum()
      this.totalOrderNum = result.data.totalOrderNum
      this.curOrderNum = result.data.curOrderNum

    },
    async getVisitNum(){
      const result = await this.$API.dashboard.getVisitNum()
      this.totalVisitNum = result.data.totalVisitNum
      this.curVisitNum = result.data.curVisitNum

    },
    async getActData(){
      const result = await this.$API.dashboard.getActData()
      this.act = result.data.act
      this.allpeople = result.data.allpeople
      this.actpeople = result.data.actpeople
